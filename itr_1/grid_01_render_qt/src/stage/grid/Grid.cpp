...
mSize(numCellsX,numCellsY){
    ...
    while (++i < mSize.y) {
        j = -1;
        while(++j < mSize.x){
            mCells += new next::Cell(Index(j,i),Vec3f(-size_2 + j, 0, -size_2 + i));
        }
    }
}
...
next::Cell* Grid::getCell(int n){
    assert( n > -1 && n < mCells.size());
    return mCells[n];
}

const next::Cell* Grid::getCell(int n) const{
    return getCell(n);
}

next::Cell* Grid::getCell(int x, int y){
    assert( x > -1 && x < mSize.x && y > -1 && y < mSize.y);
    return mCells[ y * mSize.x + x ];
}

next::Cell* Grid::getCell(const Index& index){
