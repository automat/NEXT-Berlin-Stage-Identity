...
ThemeView::ThemeView(Grid* grid, const LayoutArea& area, Oscillator* oscillator, vector<Quote>* quotes) :
    ...
    mQuotes(quotes){
        ...
        mReleaseTheDingo = false;
        
        
        const vector<next::Cell*>& gridCells = mGrid->getCells();
        // Create diverfields according to layoutarea
        for(vector<next::Cell*>::const_iterator itr = gridCells.begin(); itr != gridCells.end(); ++itr){
            const next::Cell* cell = *itr;
