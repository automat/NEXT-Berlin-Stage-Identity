...
public:
    ...
    const next::Cell* getCell(int n) const;
    next::Cell* getCell(int n);
    next::Cell* getCell(int x, int y);
    next::Cell* getCell(const Index& index);
    
    inline const vector<next::Cell*>& getCells() const{
        return mCells;
    }
