...
uniform float     uNum; //overall size
...
void main(){
    ...
    vec4 position = dataPrevPosition; //+ sin(dataIndexMap.r / (float)uNum * M_PI + uTime);

    float norm   = dataIndexMap.r / uNum;
    float angle  = norm * M_PI * 2.0 + uTime;
    float radius = 0.5;
    
    position.y = sin(norm * M_PI * 128.0) * 0.25;
    position.x = (radius - sin(norm * M_PI * 512.0) * cos(norm * M_PI * 16.0) * 0.125 ) * cos(angle);// / uNum;
    position.z = radius*sin(angle);//sin(dataIndexMap.r/uNum * M_PI + uTime);
